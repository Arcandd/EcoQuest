...
#define WIFI_SSID "James Alvin’s iPhone"
#define WIFI_PASSWORD "lordbombom"

// The API key can be obtained from Firebase console > Project Overview > Project settings.
#define API_KEY "AIzaSyBBxS1ZXP8uERrdJ-b1Wv8BSt8LOdt08VE"

// User Email and password that already registerd or added in your project.
#define USER_EMAIL "test@gmail.com"
#define USER_PASSWORD "mytest"
#define DATABASE_URL "https://eco-quest-8c27c-default-rtdb.firebaseio.com/"
...
void setup()
{
    ...
    // int v1 = Database.get<int>(aClient, "/data/airConditioner/highThreshold");
...
double prevLowThreshold = -1;
double prevHighThreshold = -1;
bool prevPower = -1;

void getAcData(){
  airConditioner.lowThreshold =  Database.get<double>(aClient, "/data/airConditioner/lowThreshold");
  airConditioner.highThreshold =  Database.get<double>(aClient, "/data/airConditioner/highThreshold");
  airConditioner.power = Database.get<bool>(aClient, "/data/airConditioner/power");
  if(
    airConditioner.lowThreshold != prevLowThreshold ||
    airConditioner.highThreshold != prevHighThreshold ||
    airConditioner.power != prevPower
  ){
    prevLowThreshold = airConditioner.lowThreshold;
    prevHighThreshold =  airConditioner.highThreshold;
    prevPower = airConditioner.power;
    printAcDataToTerminal();
  }
}

double temperature;
void getAndSetTemperatureData(){
  String incomingString;
  if (Serial.available() > 0){
    // read the incoming string from the Arduino
    incomingString = Serial.readString();
    temperature = incomingString.toDouble();
    Database.set<number_t>(aClient, "/data/airConditioner/temperature", number_t(temperature));
  }
}


void printAcDataToTerminal(){
  Serial.print(airConditioner.power);
  Serial.print(";");
  Serial.print(airConditioner.lowThreshold);
  Serial.print(";");
  Serial.print(airConditioner.highThreshold);
  Serial.println("-");
}
